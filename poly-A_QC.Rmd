...
write.table(df_pca_minus,"df_pca_minus_log.csv",row.names = FALSE,sep=",")
write.table(df_pca_plus,"df_pca_plus_log.csv",row.names = FALSE,sep=",")
...
```{r}
df_pca_minus_log<-read.csv("df_pca_minus_log.csv",header = TRUE)
df_pca_plus_log<-read.csv("df_pca_plus_log.csv",header = TRUE)

plus.pca<-pca(df_pca_plus_log[,1:44778], center = TRUE, scale =FALSE)
# plus.pca.scaled<-pca(df_pca_plus_log[,1:42339], center = TRUE, scale =TRUE)
minus.pca<-pca(df_pca_minus_log[,1:47822], center = TRUE, scale =FALSE)
# minus.pca.scaled<-pca(df_pca_minus_log[,1:44883], center = TRUE, scale =TRUE)

plus.pca.plot<- 
  Scatter_Density(object = plus.pca, 
                  batch = df_pca_plus_log$batch, 
                  title = 'Poly-A plus strand data PCA plot', 
                  batch.legend.title = 'batch')
minus.pca.plot<- 
  Scatter_Density(object = minus.pca, 
                  batch = df_pca_minus_log$batch, 
...
```{r}
#Updated PCA
df_pca_minus_updated<-df_pca_minus_log[-as.numeric(minus.outliers),]
df_pca_plus_updated<-df_pca_plus_log[-as.numeric(plus.outliers),]
```


```{r}
plus.pca_updated<-pca(df_pca_plus_updated[,1:44778], center = TRUE, scale =FALSE)

minus.pca_updated<-pca(df_pca_minus_updated[,1:47822], center = TRUE, scale =FALSE)
...
```
```{r}
x<-pca.outlier(df_pca_minus_log[,1:47822], center = TRUE, scale =FALSE, conf.level = 0.975) 
x
...
```
```{r}
p<-pca.outlier(df_pca_plus_log[,1:44778], center = TRUE, scale =FALSE, conf.level = 0.975) 
p
```
...
library(RColorBrewer)
    ...
    ## laod Data
    TPM_data <- transpose(df_pca_minus_log[,1:47822])
    rownames(TPM_data) <- colnames(df_pca_minus_log[,1:47822])
    #TPM_data = TPM_data[,-1]
