...
// Rewritten by Adam Gower, based on scripts by Josh Campbell and Yusuke Koga; modified by Jenny Empawi
...
// Retrieve and process Ensembl GTF, FASTA and BED files //////////////////////////////
...
process generateGTF {
  ...
  script:
  ensembl_gtf_file = "Homo_sapiens.${params.genome.assembly}.${params.genome.ensembl}.gtf.gz"

  """
  wget -L ftp://ftp.ensembl.org/pub/release-${params.genome.ensembl}/gtf/homo_sapiens/Homo_sapiens.${params.genome.assembly}.${params.genome.ensembl}.gtf.gz
  """
}
...
process generateBED {
  ...
  file(ensembl_bed_file) \
    into (
      ...
      generateBED_to_runRSeQCsexcheck
    )
...
process generateFASTA {
  ...
  script:
  ensembl_fasta_file = "Homo_sapiens.${params.genome.assembly}.dna.primary_assembly.fa.gz"

  """
  wget -L ftp://ftp.ensembl.org/pub/release-${params.genome.ensembl}/fasta/homo_sapiens/dna/Homo_sapiens.${params.genome.assembly}.dna.primary_assembly.fa.gz
  """
}
...
process runSTAR2pass {
  ...
  publishDir "${params.output_dir}/Samples/${indivID}/${sampleID}"
  ...
  set indivID, sampleID, file(outfile_bam) \
    into (
      ...
      runSTAR2pass_to_runRSeQCsexcheck
    )
  // Output to RSEM
...
process runRSEMprepareReference {
  ...
  rsem_fasta_file="${ensembl_fasta_file.name.replaceFirst("\\.gz", "")}"
  ...
  zcat ${ensembl_fasta_file} > ${rsem_fasta_file}
...
process runRSEMcalculateExpression {
  ...
  publishDir "${params.output_dir}/Samples/${indivID}/${sampleID}/RSEM"
...
process runFastQC {
  ...
  publishDir "${params.output_dir}/Samples/${indivID}/${sampleID}/Processing/Libraries/${libraryID}/${rgID}/FastQC"
...
process runRSeQCbamStat {
  ...
  publishDir "${params.output_dir}/Samples/${indivID}/${sampleID}/RSeQC"
...
process runRSeQCsexcheck {
  tag "Running RSeQC sexcheck for ${sampleID}"
  publishDir "${params.output_dir}/Samples/${indivID}/${sampleID}/RSeQC"
  ...
  input:
  set indivID, sampleID, bam from runSTAR2pass_to_runRSeQCsexcheck
  file(ensembl_bed_file) from generateBED_to_runRSeQCsexcheck
 
  output:
  file("${sampleID}.idxstat") \
    into (runRSeQCsexcheck_to_runMultiQCSample,
    runRSeQCsexcheckOutput
    )
...
process runMultiQCSample {
  ...
  val rseqc_sexcheck_files \
    from runRSeQCsexcheck_to_runMultiQCSample.toSortedList()
...
process createSE {
  ...
  file("*.rds") \
    into (
      runCreateSEOutput,
      createSE_to_runOutliersDetection
    )
...
process runOutliersDetection {
  tag "Running PCA, RLE and D-statistic"
  publishDir "${params.output_dir}/Output/Outliers_detection"

  module params.modules.R

  input:
  file(rds_files) from createSE_to_runOutliersDetection

  output:
  set file("*.pdf") ,file("*.rds") into runOutliersDetectionOutput
  ...
  """
  Rscript ${params.output_dir}/outlier_detection.R ${rds_files.gene} ${rds_files.isoform}
  """
 }
